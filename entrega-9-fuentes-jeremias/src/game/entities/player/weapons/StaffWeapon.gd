...
onready var staff_sfx = $StaffSfx
export (AudioStream) var attack_primary
export (AudioStream) var attack_seccondary
export (AudioStream) var attack_special
...
func update_weapon(delta: float, character, can_attack: bool = true) -> void:
	...
	mana_bar.max_value = character.max_mana
	mana_bar.value = character.mana
...
func attack(type: int) -> void:
	...
	match anim_map[type]:
		"bolt":
			staff_sfx.stream = attack_primary
		"wind":
			staff_sfx.stream = attack_seccondary
		"fire":
			staff_sfx.stream = attack_special
	staff_sfx.play()		
